...
def apply_style(profile: dict[str, Any], layout: str = "single") -> tuple[float, float]:
    ...
    cast(MutableMapping[str, Any], plt.rcParams).update(rc_params)
    return width, height
...
def _add_significance_annotation(
    ...
    ax.plot(
        [x1, x1, x2, x2], [y, bracket_y, bracket_y, y], color="black", linewidth=0.6
    )
...
def _draw_line(
    ax: Axes, frame: Any, figure: dict[str, Any], palette: list[str], kind: str
) -> None:
...
def _draw_bar(ax: Axes, frame: Any, figure: dict[str, Any], palette: list[str]) -> None:
    x, y, group = figure["x"], figure["y"], figure.get("group")
    lower, upper = figure.get("lower"), figure.get("upper")
    groups = [(None, frame)] if not group else list(frame.groupby(group, sort=False))
...
def _draw_ablation(
    ...
    _draw_bar(ax, frame, figure, palette)


def _draw_time_series(
    ax: Axes, frame: Any, figure: dict[str, Any], palette: list[str]
) -> None:
    _draw_line(ax, frame, figure, palette, "time_series")
...
def _draw_calibration(
    ...
    _draw_line(ax, frame, figure, palette, "calibration")


_DISPATCH: dict[str, Any] = {
    "line": _draw_line,
    "time_series": _draw_time_series,
...
def draw(
    ...
    if isinstance(ax, str):
        return _draw_standalone(ax, Path(frame), figure, Path(palette))
    ...
    if handler is _draw_line:
        handler(ax, frame, figure, palette, kind)
    else:
...
def _default_figure_spec(kind: str, frame: Any) -> dict[str, Any]:
    if kind in {"bar", "ablation"}:
        return {
            "type": kind,
            "x": "category",
            "y": "value",
    ...
    if kind in {"line", "time_series"}:
        return {"type": kind, "x": "x", "y": "y", "xlabel": "X", "ylabel": "Y"}
    ...
    if kind == "calibration":
        x = "value" if "value" in frame.columns else frame.columns[0]
        y = "value" if "value" in frame.columns else frame.columns[1]
        return {
            "type": kind,
            "x": x,
            "y": y,
    ...
    raise ValueError(f"Unsupported figure type: '{kind}'")


def _draw_standalone(
    kind: str,
    data_path: Path,
    ...
    spec = _default_figure_spec(kind, frame)
    draw(axis, frame, spec, _get_palette(profile))
    fig.tight_layout()
    stem = output_dir / kind
    fig.savefig(stem.with_suffix(".pdf"))
    fig.savefig(stem.with_suffix(".png"), dpi=profile.get("raster_dpi", 300))
    plt.close(fig)
    (output_dir / "caption.md").write_text("\n", encoding="utf-8")
    (output_dir / "latex_include.tex").write_text("\n", encoding="utf-8")
    (output_dir / "word_insertion.txt").write_text("\n", encoding="utf-8")
    return kind
...
def _render_figures(
    ...
    if len(figures) > 1:
        ...
        for i, spec in enumerate(figures):
            source = resolve_request_path(
                Path(request.get("_request_path", "")), spec.get("source", "")
            )
...
def main() -> int:
    ...
    copy_if_distinct(
        Path(__file__).with_name("exit_codes.py"), output / "exit_codes.py"
    )
    copy_if_distinct(
        Path(__file__).with_name("logging_config.py"), output / "logging_config.py"
    )
    copy_if_distinct(
