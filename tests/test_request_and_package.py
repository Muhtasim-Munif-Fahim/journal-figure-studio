...
def test_render_and_check_complete_publication_package(tmp_path: Path) -> None:
    ...
    analysis.write_text(
        "# Source analysis recorded for test fixture.\n", encoding="utf-8"
    )
    request = {
    ...
    subprocess.run(
        [sys.executable, str(SCRIPTS / "validate_request.py"), str(request_path)],
        check=True,
    )
    subprocess.run(
        [
            sys.executable,
            str(SCRIPTS / "render_recipe.py"),
            "--request",
            str(request_path),
        ],
        check=True,
