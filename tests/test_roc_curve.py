"""Tests for the roc_curve figure type."""
...
from scripts.render_recipe import _draw_roc_curve


def test_roc_curve_renders_for_perfect_classifier() -> None:
    fig, ax = plt.subplots()
    ...
    _draw_roc_curve(ax, frame, {"label": "label", "score": "score"}, ["#1f77b4"])
    assert len(ax.lines) >= 1
    assert ax.get_legend() is not None
    # Chance line is drawn by default.
    legend_labels = [text.get_text() for text in ax.get_legend().get_texts()]
    assert any("chance" in label for label in legend_labels)
    plt.close(fig)


def test_roc_curve_with_group_renders_multiple_curves() -> None:
    fig, ax = plt.subplots()
    rng = np.random.default_rng(0)
    rows = []
    ...
    # Two curves (one per group) plus the chance line.
    assert len(ax.lines) >= 2
    plt.close(fig)


def test_roc_curve_respects_diagonal_flag() -> None:
    fig, ax = plt.subplots()
    rng = np.random.default_rng(0)
    n = 30
    ...
    _draw_roc_curve(
        ax, frame,
        {"label": "label", "score": "score", "roc": {"diagonal": False}},
        ["#1f77b4"],
    )
    # No chance line when diagonal=False.
    legend_labels = [text.get_text() for text in ax.get_legend().get_texts()]
    assert not any("chance" in label for label in legend_labels)
    plt.close(fig)


def test_roc_curve_handles_truthy_label() -> None:
    fig, ax = plt.subplots()
    rng = np.random.default_rng(0)
    n = 30
    score = np.concatenate([rng.standard_normal(n // 2) + 1, rng.standard_normal(n // 2) - 1])
